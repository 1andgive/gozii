...
def get_image(q_id, Dict_qid2vid,
              data_root='D:\\Data_Share\\Datas\\VQA_COCO\\Images\\test2015'):  # q_id => single scalar
...
def check_captions(caption_generator, dataloader, Dict_qid2vid, vocab, save_fig_loc, x_method_, t_method_, s_method_):
    ...
    for v, b, q, i in iter(dataloader):
        ...
        with torch.no_grad():
            ...
            generated_captions, logits, att = caption_generator.generate_caption(v, b, q, t_method=t_method_,
                                                                                 x_method=x_method_, s_method=s_method_)
        ...
        RelScore = Relev_Check(captions_list[0], q, answer_list[0], caption_generator.BAN.module.w_emb,
                               dataloader.dataset.dictionary)
        ...
        if (s_method_ == 'BestOne'):
            tmp_fig = showAttention(question_list[0], img_list[0], answer_list[0], att[0, :, :, :], b[0, :, :4],
                                    captions_list[0], RelScore, display=False)
        elif (s_method_ == 'BeamSearch'):
            tmp_fig = showAttention(question_list[0], img_list[0], answer_list[0], att[0, :, :, :], b[0, :, :4],
                                    captions_list[:len(generated_captions)], RelScore, display=False,
                                    NumBeams=len(generated_captions))
        plt.savefig(os.path.join(save_fig_loc, 'model{}'.format(args.model_num), t_method_, x_method_, s_method_,
                                 '{}.png'.format(i.item())))
        plt.close(tmp_fig)
...
if __name__ == '__main__':
    ...
    def process(args, model, eval_loader, Dict_qid2vid):
        ...
        if not os.path.exists(
                os.path.join(args.save_fig_loc, 'model{}'.format(args.model_num), args.t_method, args.x_method,
                             args.s_method)):
            os.makedirs(os.path.join(args.save_fig_loc, 'model{}'.format(args.model_num), args.t_method, args.x_method,
                                     args.s_method))
        check_captions(caption_generator, eval_loader, Dict_qid2vid, vocab, args.save_fig_loc, args.x_method,
                       args.t_method, args.s_method)
